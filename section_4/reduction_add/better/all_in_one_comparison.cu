// This program has a fatal problem:
// There are dependencies between different algorithms.
// Actually for the two algorithms running on GPU,
// the latter executed one is always faster.
...
float gpu_sum_reduction(float *input, int vector_size, int choice);
__global__ void gpu_sum_reduction_kernel_basic(float *d_input, float *d_output);
__global__ void gpu_sum_reduction_kernel_better(float *d_input, float *d_output);
...
int main()
{
    ...
    printf("GPU better start ...\n");
    gettimeofday(&begin, NULL);
    float gpu_res = gpu_sum_reduction(input, vector_size, 1);
    gettimeofday(&end, 0);
    elapsed_sec = (double)(end.tv_sec - begin.tv_sec) + (double)(end.tv_usec - begin.tv_usec) / 1000000.0;
    printf("gpu_res (better): %f\n", gpu_res);
    printf("gpu_time (better): %lf sec\n", elapsed_sec);
    ...
    // launch the function using GPU
    printf("GPU basic start ...\n");
    gettimeofday(&begin, NULL);
    gpu_res = gpu_sum_reduction(input, vector_size, 0);
    gettimeofday(&end, 0);
    elapsed_sec = (double)(end.tv_sec - begin.tv_sec) + (double)(end.tv_usec - begin.tv_usec) / 1000000.0;
    printf("gpu_res (basic): %f\n", gpu_res);
    printf("gpu_time (basic): %lf sec\n", elapsed_sec);
...
float gpu_sum_reduction(float *input, int vector_size, int choice)
{
    ...
    while (vector_size >= BLOCK_SIZE * 2)
    {
        ...
        if (choice == 0)
            gpu_sum_reduction_kernel_basic<<<num_blocks, BLOCK_SIZE>>>(d_input, d_output);
        else
            gpu_sum_reduction_kernel_better<<<num_blocks, BLOCK_SIZE>>>(d_input, d_output);
...
__global__
void gpu_sum_reduction_kernel_basic(float *d_input, float *d_output)
{
...
__global__
void gpu_sum_reduction_kernel_better(float *d_input, float *d_output)
{
    // calculate the global_thread_idx
    int block_input_start_idx = blockIdx.x * blockDim.x * 2;
