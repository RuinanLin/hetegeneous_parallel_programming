...
Graph::Graph(std::string prefix, bool use_dag, bool directed, 
             bool use_vlabel, bool use_elabel, bool need_reverse, bool bipartite, int my_id) :
...
#ifdef USE_METIS
  std::cout << "Using metis ...\n";
  std::string global_graph_path;
  if (i != string::npos) global_graph_path = inputfile_path.substr(0, i); // ../../../../inputs/$name
  i = global_graph_path.rfind('/', global_graph_path.length());
  if (i != string::npos) name_ = global_graph_path.substr(i+1); // $name
  prefix = prefix + std::to_string(my_id);  // ../../../../inputs/$name/metis/pgraph0
#else
  ...
  assert(f_meta);
#ifdef USE_METIS
  f_meta >> n_vertices >> n_real_vertices >> n_edges >> max_subg_nv >> max_subg_ne >> max_degree;
#else
...
#ifdef USE_METIS
  read_file(prefix + ".vertex_list.bin", vertex_list, n_real_vertices);
  read_file(prefix + ".part.bin", part, n_vertices);
#endif
