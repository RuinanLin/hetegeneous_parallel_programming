...
protected:
    ...
    vidType *push_buffer;       // a scratchpad, storing the forming pushed data
...
public:
    ...
    void init(Graph &hg) {
        ...
        CUDA_SAFE_CALL(cudaMalloc((void **)&pull_buffer, n_total_warps_in_a_grid * max_degree * sizeof(vidType)));  // TODO: it is too big
        CUDA_SAFE_CALL(cudaMalloc((void **)&push_buffer, n_total_warps_in_a_grid * (3+2*max_degree) * sizeof(vidType)));    // TODO: it is too big
    ...
    inline __device__ __host__ vidType *get_my_push_buffer(int warp_id) { return push_buffer + warp_id * (3+2*max_degree); }
};