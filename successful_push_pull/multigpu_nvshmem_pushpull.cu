...
void TCSolver(Graph &g, uint64_t &total, int n_partitions, int chunk_size) {
    ...
    // perpare the data structures
    GraphNVSHMEM d_graph(g, mype, n_total_warps_in_a_grid);
    // int num_sender_recver_blocks = nblocks / 9;     // TODO: change it back to normal
    // int num_sender_recver_warps = num_sender_recver_blocks * nwarps;
    // int dest_warp_group_size = num_sender_recver_warps / (ndevices - 1);
    // num_sender_recver_warps = dest_warp_group_size * (ndevices - 1);
    // std::cout << "num_sender_recver_warps = " << num_sender_recver_warps << "\n";
    // NVSHMEMBuffer d_push_buffer(4+2*g.get_max_degree(), num_sender_recver_warps);
    NVSHMEMBuffer d_push_buffer(4+2*g.get_max_degree(), 456);
    ...
    int h_debug_lock = 0;
    int *d_debug_lock = (int *)nvshmem_malloc(sizeof(int));
    CUDA_SAFE_CALL(cudaMemcpy(d_debug_lock, &h_debug_lock, sizeof(int), cudaMemcpyHostToDevice));
    Timer t;
    t.Start();
    warp_vertex_nvshmem<<<nblocks, nthreads>>>(d_count, d_graph, d_push_buffer, d_debug_lock);
    CUDA_SAFE_CALL(cudaDeviceSynchronize());
    t.Stop();
