...
protected:
  ...
  int core_length_;
  vidType n_real_vertices;      // if it is a metis subgraph, how many vertices are really in this subgraph
  ...
  vidType *vertex_list; // vertices in this partition
  idx_t *part;          // each vertex belongs to which partition
#endif

public:
  Graph(std::string prefix, bool use_dag = false, bool directed = false,
        bool use_vlabel = false, bool use_elabel = false, 
        bool need_reverse = false, bool bipartite = false, int my_id = 0);
  ...
  void set_n_real_vertices(vidType number) { n_real_vertices = number; }
  vidType get_n_real_vertices() { return n_real_vertices; }

#ifdef USE_METIS
  vidType get_max_subg_nv() { return max_subg_nv; }
  eidType get_max_subg_ne() { return max_subg_ne; }
  vidType *get_vertex_list() { return vertex_list; }
