...
float N[IMAGE_SIZE * IMAGE_SIZE];
float M[MASK_SIZE * MASK_SIZE];
...
void cuda_conv_2d_kernel(float *d_P, float *d_N, const float * __restrict__ d_M)
{
    ...
    if (index_i_row >= 0 && index_i_row < IMAGE_SIZE && index_i_col >= 0 && index_i_col < IMAGE_SIZE)
        Ns[ty][tx] = d_N[index_i_row * IMAGE_SIZE + index_i_col];
    else
        Ns[ty][tx] = 0.0f;
    __syncthreads();
    ...
    if (ty < O_TILE_WIDTH && tx < O_TILE_WIDTH)
    {
        ...
        for (int i = 0; i < MASK_SIZE; i++)
            for (int j = 0; j < MASK_SIZE; j++)
                res += Ns[ty + i][tx + j] * d_M[i * MASK_SIZE + j];
...
void cuda_conv_2d(float *h_P, float *h_N, float *h_M)
{
    ...
    cudaMalloc((void **)&d_N, IMAGE_SIZE * IMAGE_SIZE * sizeof(float));
    cudaMemcpy(d_N, h_N, IMAGE_SIZE * IMAGE_SIZE * sizeof(float), cudaMemcpyHostToDevice);
    cudaMalloc((void **)&d_M, MASK_SIZE * MASK_SIZE * sizeof(float));
    cudaMemcpy(d_M, h_M, MASK_SIZE * MASK_SIZE * sizeof(float), cudaMemcpyHostToDevice);
            ...
            for (int m_row = 0; m_row < MASK_SIZE; m_row++)
                for (int m_col = 0; m_col < MASK_SIZE; m_col++)
                    if (index_i_row + m_row >= 0 && index_i_row + m_row < IMAGE_SIZE && index_i_col + m_col >= 0 && index_i_col + m_col < IMAGE_SIZE)
                        ref += h_N[(index_i_row + m_row) * IMAGE_SIZE + (index_i_col + m_col)] * h_M[m_row * MASK_SIZE + m_col];
