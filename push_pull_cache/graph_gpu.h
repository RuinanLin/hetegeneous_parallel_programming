...
public:
  ...
  void init_nvshmem(Graph &g, int mype, size_t message_buffer_size) {
    ...
    CUDA_SAFE_CALL(cudaMemcpy(d_rowptr, g.out_rowptr(), (num_vertices+1) * sizeof(eidType), cudaMemcpyHostToDevice));

    // allocate colidx
    eidType subg_ne = g.E();
    CUDA_SAFE_CALL(cudaMalloc((void **)&d_colidx, subg_ne * sizeof(vidType)));
    CUDA_SAFE_CALL(cudaMemcpy(d_colidx, g.out_colidx(), subg_ne * sizeof(vidType), cudaMemcpyHostToDevice));

    // allocate vertex_list
    vidType subg_nv = g.get_n_real_vertices();
    CUDA_SAFE_CALL(cudaMalloc((void **)&d_vertex_list, subg_nv * sizeof(vidType)));
    CUDA_SAFE_CALL(cudaMemcpy(d_vertex_list, g.get_vertex_list(), subg_nv * sizeof(vidType), cudaMemcpyHostToDevice));
    ...
    CUDA_SAFE_CALL(cudaMemcpy(d_part, g.get_metis_part(), num_vertices * sizeof(idx_t), cudaMemcpyHostToDevice));
  }
};
