...
void PartitionedGraph::metis_partition() {
  num_subgraphs = num_vertex_chunks;
  std::cout << "Metis partitioning into " << num_subgraphs << " subgraphs\n";
  subgraphs.resize(num_subgraphs);
  vidType nv = g->V();
  eidType ne = g->E();
  ...
  for (eidType e = 0; e < ne; e++) {
    adjncy[e] = (idx_t)edges[e];
  }

  METIS_PartGraphKway(&nvtxs, &ncon, xadj, adjncy, NULL, NULL, NULL, &nparts, NULL, NULL, NULL, &objval, part);

  verts_of_clusters.resize(num_subgraphs);        // records each partition's vertices
  std::vector<eidType> num_edges_each_partition;  // records each partition's edge_num
  num_edges_each_partition.resize(num_subgraphs, 0);
  for (vidType v = 0; v < nv; v++) {
    verts_of_clusters[part[v]].push_back(v);
...
// write the partitioned graph into files
void PartitionedGraph::metis_write_into_file() {
  partitioned_file_path = g->get_inputfile_path() + '/';
  std::cout << "writing to path: " << partitioned_file_path << "\n";

  // get the max_subg_nv and max_subg_ne
  vidType max_subg_nv = 0;
  eidType max_subg_ne = 0;
  for (int sg_id = 0; sg_id < num_subgraphs; sg_id++) {
    auto &subg = *get_subgraph(sg_id);
    auto subg_nv = subg.get_n_real_vertices();
  ...
  for (int sg_id = 0; sg_id < num_subgraphs; sg_id++) {
    auto &subg = *get_subgraph(sg_id);
    std::cout << "writing subgraph " << sg_id << "\n";

    // meta
    ofstream p_meta(partitioned_file_path+"metis/pgraph"+std::to_string(sg_id)+".meta.txt");
    assert(p_meta);
    p_meta << subg.V() << "\n" << subg.get_n_real_vertices() << "\n" << subg.E() << "\n" << max_subg_nv << "\n" << max_subg_ne << "\n" << g->get_max_degree() << "\n";
    p_meta.close();

    // vertices
    ofstream p_vertices(partitioned_file_path+"metis/pgraph"+std::to_string(sg_id)+".vertex.bin", ios::out | ios::binary);
    assert(p_vertices);
